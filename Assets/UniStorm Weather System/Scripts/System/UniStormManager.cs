...
namespace UniStorm
{
    public class UniStormManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Smoothly transitions UniStorm's Time
        /// </summary>
        public void FastForwardTo(int Hour, int Minute)
        {
            int currentHour = UniStormSystem.Instance.Hour;
            
            bool hourOverMidnight = currentHour > Hour;

            int hourToMidnight = hourOverMidnight ? 24 - currentHour : Hour - currentHour;
            int hourFromMidnight = hourOverMidnight ? Hour : 24 - Hour;

            hourToMidnight /= 2;
            hourFromMidnight /= 2;
            
            if(hourOverMidnight)
                DOTween.To(() => currentHour, x => SetTime(x, Minute), 24, hourToMidnight)
                    .OnUpdate(() => SetTime(currentHour, Minute))
                    .OnComplete(() => SetTime(0, 0))
                    .OnComplete(() => DOTween.To(() => currentHour, x => SetTime(x, Minute), Hour, hourFromMidnight)
                        .OnUpdate(() => SetTime(currentHour, Minute)));
            else
            {
                DOTween.To(() => currentHour, x => currentHour = x, Hour, hourToMidnight)
                    .OnUpdate(() => SetTime(currentHour, Minute));
            }
